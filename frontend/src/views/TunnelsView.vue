...
import { h } from 'vue'
import { tunnelList, tunnelCreate, tunnelDelete, tunnelDiagnose, nodeList } from '@/api'

const message = useMessage()
const rows = ref<any[]>([])
const nodes = ref<any[]>([])
const showModal = ref(false)
const form = ref({ name: '', type: 1, flow: 1, trafficRatio: 1, inNodeId: [] as any[], inIp: '' })
    ...
    <n-modal v-model:show="showModal" preset="card" title="新建隧道(端口转发)" style="width: 480px">
      <n-form label-placement="left" label-width="90px">
        ...
        <n-form-item label="入口节点">
          <n-select v-model:value="form.inNodeId" multiple :options="nodes.filter((n) => n.status === 1).map((n) => ({ label: `${n.name} (${n.serverIp})`, value: { nodeId: n.id } }))" />
        </n-form-item>
